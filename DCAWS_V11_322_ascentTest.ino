...
// Changes from previous version: changed logging style, finished abort state, added new T gain and depth offset,
    // reinitialize sample mission bools so all three samples should work, SD working, sampling verifiied with hardcode depth
// include libraries
...
int targetCount = 1;
int targetDepth;
int targetDepth1 = 0.5 + OFFSET;
int targetDepth2 = 1.0 + OFFSET;
int targetDepth3 = 1.5 + OFFSET;
float pidError = 0;
...
enum {GPS_DRIFT, ASCENT, SAMPLE_MISSION, GPS_FINAL, ABORT, TEST} state;
...
void loop()
{
  ...
  switch (state)
  {
    ...
    case ASCENT:
      ascend();
      if (/*avgDepth*/pidDepth <= .25 && prevDepth <= .25)
      {
        surfaceCount++;
